...
1. As a Hopkins student, I would login with my Google account, so I can be freed from tedious registration process of personal email address and verification of "I am a Hopkins student"
...
4. As a person who book a drive on different purposes, including booked-ahead ride share to airport and instantly needed ride share, I want to have two types of rides being classified well with tags so I could search for the drive desired more effectively.
5. As a client of HopShare, I would like to complete all of my connections in a secure private chat window integrated in the app with security verification, so I will be less likely to be exposed to scamming and cheating.

### Nice to have

1. As an app user, I want the mobile-friendly version be provided so I can use this app anywhere and anytime
...
4. Critical backend routes must have automated integration tests to catch regressions

## Our plan for this iteration
1. Integrate the map view with the search and filter panel so users can visually browse and filter rides on the same screen
2. Implement direct messaging (DM) so users can privately message each other outside of ride group chats, with unread badges displayed in the messages tab
3. Add a starred rides feature so users can bookmark rides they are interested in and view them on a dedicated starred page
4. Allow ride posters to set a maximum rider count on their posts so the group size is clearly communicated and enforced
5. Add a price estimation link to ride posts so riders have a quick reference for splitting costs
6. Require friend request approval so friendship is mutual and both parties must confirm before being added to each other's friend list
7. Restrict removed riders to read-only chat access after they leave or are removed from a ride, preserving conversation history without allowing further participation
8. Redesign the landing page with a cleaner layout and move the Google login button directly onto the landing page
9. Write backend integration tests for the critical routes (create post, join post, Google auth, respond to notification) using a mock database
